...
class GradeTables:
    def __init__(
        ...
        try:
            self.term = (
                "T"
                + self.grades_df.loc[
                    self.grades_df["GR Term"].str.contains("T"), "GR Term"
                ]
                .str.split(pat="T")
                .max()[1]
            )
            ...
            self.term = ""
        try:
            self.campus = self.homerooms_df.loc[0, "School"]
        except:
            self.campus = ""
    ...
    def _clean_grades_df(self):
        try:
            self.grades_df = self.grades_df.loc[
                self.grades_df["Enrollment"] == "active", :
            ]
        except:
            print("No Enrollment data")
        try:
            self.grades_df = self.grades_df.loc[
                self.grades_df["GR Term"] == self.term, :
            ]
        except:
            print("No Term Data")
        ...
        self.grades_df["Duplicates"] = self.grades_df.duplicated(
            subset=["Student Number", "Crs Name"], keep="last"
        )
    ...
    def _clean_homerooms_df(self):
        self.homerooms_df = self.homerooms_df.loc[
            self.homerooms_df["Enroll Status"] == "Active", :
        ]
        self.homerooms_df = self.homerooms_df.loc[
            :, ["Student Number", "Student", "Grade", "Home_Room"]
        ]
        self.homerooms_df.loc["Home_Room"] = self.homerooms_df["Home_Room"].str.lower()
        self.homerooms_df.loc[
            self.homerooms_df["Home_Room"].str.lower() == "michigan", "Home_Room"
        ] = "MICHIGAN STATE"
        self.homerooms_df["Home_Room"] = self.homerooms_df["Home_Room"].str.capitalize()

    def _aggregate_data(self):
        self.grades_df = pd.merge(
            self.grades_df,
            self.homerooms_df[["Student Number", "Home_Room"]],
            "left",
            ["Student Number"],
        )
        self.grades_df = self.grades_df[
            ["Home_Room"] + self.grades_df.columns[:-1].to_list()
        ]

    def _init_master_df(self) -> pd.DataFrame:
        df = pd.DataFrame({})
        df[["Home_Room", "Student", "Grade"]] = self.grades_df.loc[
            :, ["Home_Room", "Student", "Grade"]
        ].drop_duplicates()
        return df
    ...
    def _init_courses(self):
        ...
        for crs in self.courses:
            ...
            if iscrs == -1:
                ...
                self.grades_df = self.grades_df.loc[
                    self.grades_df["Crs Name"] != crs, :
                ]

    def _create_master_df(self):
        for crs in self.courses:
            df = self.grades_df.loc[
                self.grades_df["Crs Name"] == crs, ["Student", "Pct"]
            ]
    ...
    def _write_mgr_to_file(self):
        ...
        honor_roll_df = create_honor_roll_table(
            self.master_df.columns.get_loc("Honor Roll Status")
        )
        honor_roll_df.to_excel(
            writer, "Summary", header=False, index=False, startrow=0, startcol=10
        )
    ...
    def _clean_grades_df_for_output(self, df: pd.DataFrame):
        ...
        reordered_cols = (
            ["Home_Room", "Student", "#_Failing"]
            + df.columns.tolist()[2:-3]
            + ["Average", "Honor_Roll_Status"]
        )
        ...
        for sheet_name in self.sheets_list:
            ...
            if col == "Grade":
                sheet_df = self.master_df.loc[
                    ...
                    & (
                        self.master_df["Home_Room"].isin(self.resource_homerooms)
                        == False
                    ),
                    :,
                ]
                courses = (
                    self.grades_df.loc[
                        ...
                        & (
                            self.grades_df["Home_Room"].isin(self.resource_homerooms)
                            == False
                        ),
                        "Crs Name",
                    ]
                ...
                courses = (
                    self.grades_df.loc[self.grades_df[col] == value, "Crs Name"]
                    .unique()
                    .tolist()
                )
            ...
            for column in dupes:
                ...
                try:
                    sheet_df[column] = (
                        temp_df.iloc[:, [0, 2]].fillna(0)
                        + temp_df.iloc[:, [1, 2]].fillna(0)
                    )[column]
                except:
                    print("Error")
    ...
    def _slice_and_store_grade_sheets(self):
        ...
        for grade in grades_list:
            ...
            homerooms_list = (
                df.loc[df["Home_Room"].isnull() == False, "Home_Room"].unique().tolist()
            )
            ...
            for hr in homerooms_list:
                if (
                    len(
                        self.master_df.loc[
                            self.master_df["Home_Room"] == hr, "Grade"
                        ].unique()
                    )
                    > 1
                ):
                    max = self.master_df.loc[
                        self.master_df["Home_Room"] == hr, "Grade"
                    ].max()
    ...
    def _determine_sheet_names(self, names: list):
        ...
        for name in names:
            temp = str(name)
            if temp == "0":
                sheet_names.append("Kindergarten")
            elif temp == "1":
                sheet_names.append("1st Grade")
            elif temp == "2":
                sheet_names.append("2nd Grade")
            elif temp == "3":
                sheet_names.append("3rd Grade")
            elif temp == "4":
                sheet_names.append("4th Grade")
            elif temp == "5":
                sheet_names.append("5th Grade")
            elif temp == "6":
                sheet_names.append("6th Grade")
            elif temp == "7":
                sheet_names.append("7th Grade")
            elif temp == "8":
                sheet_names.append("8th Grade")
            elif temp == "9":
                sheet_names.append("9th Grade")
            elif temp == "10":
                sheet_names.append("10th Grade")
            elif temp == "11":
                sheet_names.append("11th Grade")
            elif temp == "12":
                sheet_names.append("12th Grade")
            else:
                sheet_names.append(temp.upper())
        return sheet_names
...
def get_crs(crs: str) -> str:
    ...
    if "homework" in crs:
        return -1
    if "ela intervention" in crs:
        return "ELA Intervention"
    if "ela" in crs:
        return "ELA"
    if "kindergarten " in crs:
        return crs.replace("kindergarten ", "").title()
    if "1st grade " in crs:
        return crs.replace("1st grade ", "").title()
    if "2nd grade " in crs:
        return crs.replace("2nd grade ", "").title()
    if "3rd grade " in crs:
        return crs.replace("3rd grade ", "").title()
    if "4th grade " in crs:
        return crs.replace("4th grade ", "").title()
    ...
    if "algebra" in crs:
        return "Algebra"
    if "computer science" in crs:
        return "Computer Science"
    if "science" in crs:
        return "Science"
    if "humanities" in crs:
        return "Humanities"
    if "english" in crs:
        return "ELA"
    if "reading comprehension" in crs:
        return "Reading Comprehension"
    if "reading" in crs:
        return "Reading"
    if "writing" in crs:
        return "Writing"
    if "social studies" in crs:
    ...
    if "senior seminar" in crs:
        return "Senior Seminar"
    if "calculus" in crs:
        return "Calculus"
    if "statistics" in crs:
        return "Statistics"
    if "literature" in crs:
        return "Literature"
    if "government" in crs:
        return "Government"

    if "college and career readiness" in crs:
        return "College and Career Readiness"
