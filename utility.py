...
def create_summary_sheet(sheets_list) -> pd.DataFrame:
    """Writes summary sheet for mgr using excel formulas
    ...
    for sheet in sheets_list: 
        if sheet[1] == "Grade": summary_rows.append(int(sheet[0]))

    summary_rows.append("Totals")

    summary_data = {key: [None for x in range(0, len(summary_rows))] for key in SUMMARY_COLS}
    ...
    for grade in summary_rows[:-1]:
        summary_df.loc[summary_df['Grade']==grade, 'Failing 0'] = (
            "=COUNTIFS('Full School MGR'!D2:D1000, \"=0\", 'Full School MGR'!A2:A1000, \"={}\")".format(grade)
        )
        summary_df.loc[summary_df['Grade']==grade, 'Failing 1'] = (
            "=COUNTIFS('Full School MGR'!D2:D1000, \"=1\", 'Full School MGR'!A2:A1000, \"={}\")".format(grade)
        )
        summary_df.loc[summary_df['Grade']==grade, 'Failing 2+'] = (
            "=COUNTIFS('Full School MGR'!D2:D1000, \">=2\", 'Full School MGR'!A2:A1000, \"={}\")".format(grade)
        )

        # write total grade levels enrolled
        summary_df.loc[summary_df['Grade']==grade, 'Total Enrolled'] = (
            "=COUNTIF('Full School MGR'!A2:A1000, \"={}\")".format(grade)
        )

        # write percentage failing 0
        summary_df.loc[summary_df['Grade']==grade, 'Failing 0 - Pct'] = (
            "=B{}/I{}".format(
        ...
        summary_df.loc[summary_df['Grade']==grade, 'Failing 1 - Pct'] = (
            "=D{}/I{}".format(
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2,
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2
            ) 
        ...
        summary_df.loc[summary_df['Grade']==grade, 'Failing 2+ - Pct'] = (
            "=F{}/I{}".format(
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2,
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2
            ) 
        ...
        summary_df.loc[summary_df['Grade']==grade, 'Pass Rate'] = (
            "=(B{}+D{})/I{}".format(
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2,
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2,
                summary_df.loc[summary_df["Grade"] == grade].index[0]+2
...
def create_honor_roll_table(honor_roll_col: int) -> pd.DataFrame:
    ...
    honor_roll_df.loc[honor_roll_df['name']=='High Honor Roll', 'total'] = (
        "=COUNTIF('Full School MGR'!{}2:{}1000, \"=High Honor Roll\")".format(col_letter, col_letter)
    )
    honor_roll_df.loc[honor_roll_df['name']=='Honor Roll', 'total'] = (
        "=COUNTIF('Full School MGR'!{}2:{}1000, \"=Honor Roll\")".format(col_letter, col_letter)
    )
    honor_roll_df.loc[honor_roll_df['name']=='Total', 'total'] = (
        "=L1+L2"
    )
    ...
    for i in range(0, num_row):
        average_formula_list.append(
            '=ROUND(AVERAGE({}{}:{}{})/100,3)'.format(
                num_row_to_letter(df.columns.get_loc("# Failing")+2),       # first course col number
                i+2,                                        # student row number
                num_row_to_letter(df.columns.get_loc("Average")),       # last course col number
                i+2,                                        # student column number
            ))

        honor_roll_formula_list.append(
            '=IF({}{} >= 0.93, "High Honor Roll", IF({}{} >= 0.85, "Honor Roll",""))'.format(
                num_row_to_letter(df.columns.get_loc("Average")+1),       # first course col number
                i+2,                                        # student row number
                num_row_to_letter(df.columns.get_loc("Average")+1),       # first course col number
                i+2,                                        # student row number
            ))

        failing_formula_list.append('=COUNTIF({}{}:{}{}, "<70")'.format(
                num_row_to_letter(df.columns.get_loc("# Failing")+2),       # first course col number
                i+2,                                        # student row number
                num_row_to_letter(df.columns.get_loc("Average")),       # last course col number
                i+2,                                           # student column number
...
def format_grades(df: pd.DataFrame, sheet_name: str, writer: pd.ExcelWriter):
    ...
    grades_range =  num_row_to_letter(df.columns.get_loc("# Failing")+2) +"2:" + num_row_to_letter(df.columns.get_loc("Average")) + str(max_row+1)
    average_range = num_row_to_letter(df.columns.get_loc("Average")+1) + "2:" + num_row_to_letter(df.columns.get_loc("Average")+1) + str(max_row+1)
    failing_range = num_row_to_letter(df.columns.get_loc("# Failing")+1) + "2:" + num_row_to_letter(df.columns.get_loc("# Failing")+1) + str(max_row+1)
    honor_roll_range = num_row_to_letter(df.columns.get_loc("Honor Roll Status")+1) + "2:" + num_row_to_letter(df.columns.get_loc("Honor Roll Status")+1) + str(max_row+1)
    header_range = "A1:" + num_row_to_letter(df.columns.get_loc("Honor Roll Status")+1) + "1"
...
def format_summary(df: pd.DataFrame, sheet_name: str, writer: pd.ExcelWriter):
    ...
    worksheet.conditional_format(
        'K2:L3',
        {'type': 'no_errors',
        'format': total_honor_roll_format}
    )
